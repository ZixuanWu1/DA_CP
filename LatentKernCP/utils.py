...
def ternary_heatmap_all_methods(xy, cover_dict, vmin, vmax, method_names=None, cmap=plt.cm.RdBu):
    def kde_smooth_conditional(xy, labels, Xg, Yg):
        coords = xy.T
        weights = labels
        kde_num = gaussian_kde(coords, weights=weights, bw_method="scott")
        kde_den = gaussian_kde(coords, weights=None, bw_method="scott")
        pos = np.vstack([Xg.ravel(), Yg.ravel()])
        f_num = kde_num(pos)
        f_den = kde_den(pos)
        return np.divide(f_num, f_den, out=np.zeros_like(f_num), where=f_den > 1e-8).reshape(Xg.shape)
    ...
    y_vals = np.linspace(0, np.sqrt(3)/2, grid_size)
    Xg, Yg = np.meshgrid(x_vals, y_vals)
    mask = (Yg <= np.sqrt(3)*Xg) & (Yg <= -np.sqrt(3)*Xg + np.sqrt(3))
    ...
    for i, method in enumerate(method_names):
        smoothed = kde_smooth_conditional(xy, cover_dict[method], Xg, Yg)
        smoothed[~mask] = np.nan

        ax = axs[i]
        im = ax.imshow(smoothed, extent=[0, 1, 0, np.sqrt(3)/2],
...
def wsc_unbiased(X, cover, delta=0.2, M=1000, test_size=0.75, random_state=2020, verbose=False):
